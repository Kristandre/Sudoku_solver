...
public class Sudoku {
    ...
    public static class CSP {
        ...
        public VariablesToDomainsMapping backtrack(VariablesToDomainsMapping assignment) {
            ...
            //Find a variable that has not gotten its final value yet
            String unassignedKey = selectUnassignedVariable(assignment);
            //Create deep copies of the assignment with all possible values for the unassigned variable
            for(String domainValue:assignment.get(unassignedKey)){
                VariablesToDomainsMapping assignmentCopy = this.deepCopyAssignment(assignment);
                assignmentCopy.get(unassignedKey).clear();
                assignmentCopy.get(unassignedKey).add(domainValue);

                //inference removes all invalid solutions
                if(this.inference(assignmentCopy, this.getAllArcs())){
        ...
        public boolean inference(VariablesToDomainsMapping assignment, ArrayList<Pair<String>> queue) {
            while(!queue.isEmpty()){
                Pair<String> temp = queue.get(0);
                queue.remove(0);

                //revise removes Pairs that is not constraints
                if(revise(assignment, temp.x, temp.y)){
                    if (assignment.get(temp.x).isEmpty()) return false;
                    ArrayList<Pair<String>> neighbours = this.getAllNeighboringArcs(temp.x);
                    //If assignment was revised add all neighbouringArcs to the queue
